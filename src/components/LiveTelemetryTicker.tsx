...
export function LiveTelemetryTicker({ initialIncidents, onSelectIncident }: Props) {
  ...
  return (
    <div className="bg-slate-950 text-slate-200 border-b border-slate-800 px-6 py-2.5 flex items-center justify-between text-xs shrink-0">
      <div className="flex items-center gap-3 overflow-hidden">
        <div className="flex items-center gap-1.5 bg-indigo-500/20 text-indigo-400 px-2.5 py-0.5 rounded-full font-bold uppercase text-[10px] tracking-wider border border-indigo-500/30 shrink-0">
        ...
        {current && (
          ...
          >
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
              current.severity === 'Critical' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40' :
              current.severity === 'High' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' :
              'bg-slate-800 text-slate-300'
            }`}>
              {current.severity}
            </span>
            <span className="font-bold text-slate-100 group-hover:underline">{current.title}</span>
            <span className="text-slate-500 truncate hidden md:inline">({current.businessUnit} • {current.timeAgo})</span>
